...
def generateGs(ngx,ngy,ngz,a):
    Glist = []
    for nz in np.arange(-ngz//2, (ngz//2)+1):
        for ny in np.arange(-ngy//2, (ngy//2)+1):
            for nx in np.arange(-ngx//2, (ngx//2)+1):
...
def Hamiltonian(kx, ky, kz, gvecs, f_factors, tau):
    M = gvecs.shape[0]
    ...
    for i in range(M):
        for j in range(M):
            if(i==j): 
                ...
                # H[i,j] = V(1e-2, gvecs[i]- gvecs[j], tau)
                H[i,j] = V(1e-2, gvecs[i], tau)
    return H
